		...
		<div class="span12">              
            <ul id="MainTabs" class="nav nav-tabs">
                <li title="Lista de indicadores" class="informacion">
                    <a data-target="#waza" data-toggle="tab" href="<%=request.getContextPath()%>/IndicadorApoyo?opt=8&idfilial=<%if(fiU!=null){%><%=fiU.getIdfilial()%><%}else{%>0<%}%>&ideapfacultad=<%if(eU!=null){%><%=eU.getIdeapfacultad()%><%}else{%>0<%}%>&idfilialfacultad=<%if(faU!=null){%><%=faU.getIdfilialfacultad()%><%}else{%>0<%}%>"><div class="icon-arrow-right"></div>&nbsp;Indicadores</a>
                </li>   
            </ul>
