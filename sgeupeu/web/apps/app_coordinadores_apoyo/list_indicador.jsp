...
function validarNumeroAvance(numero){
    if(isNaN($("#prependedInput"+numero).val())){        
       alert("Se necesita un numero positivo o el valor cero");
        ...
        }else{ $("#claseform2"+numero).submit(); $("#btnavance"+numero).attr('disabled', true);  }
        }
}
</script>   
 <%
 ...
 int periodoVigente= Integer.parseInt(request.getParameter("periodoVigente")==null?"0": request.getParameter("periodoVigente"));
 int ejeVigente= Integer.parseInt(request.getParameter("ejeVigente")==null?"0": request.getParameter("ejeVigente"));
 
 
 %> 
 <div class="row-fluid">
     <div class="span3">
         <form id="periodo" name="periodo"  action="<%=request.getContextPath()%>/IndicadorApoyo?opt=2&idfilial=<%if(fiU!=null){%><%=fiU.getIdfilial()%><%}else{%>0<%}%>&ideapfacultad=<%if(eU!=null){%><%=eU.getIdeapfacultad()%><%}else{%>0<%}%>&idfilialfacultad=<%if(faU!=null){%><%=faU.getIdfilialfacultad()%><%}else{%>0<%}%>&idtemporadaejeestrategico=<%if(eje!=null){%><%=eje.getIdtemporadaejeestrategico()%><%}else{%>0<%}%>" > 
             <label>Periodo: <strong> <%if(idperidoM!=null){%><%=idperidoM.getPeriodo()%><%}else{%>-<%}%></strong></label>
          <p>  
          <select name="idperiodometa" class="span7" >
             <option value="0" <% if(periodoVigente==0){out.print("selected");} %>>-- --</option>  
            <%
     ...
     <form id="eje" name="eje"  action="<%=request.getContextPath()%>/IndicadorApoyo?opt=24&idfilial=<%if(fiU!=null){%><%=fiU.getIdfilial()%><%}else{%>0<%}%>&ideapfacultad=<%if(eU!=null){%><%=eU.getIdeapfacultad()%><%}else{%>0<%}%>&idfilialfacultad=<%if(faU!=null){%><%=faU.getIdfilialfacultad()%><%}else{%>0<%}%>&idperiodometa=<%=idperidoM.getIdperiodometa()%>" >
         <label>Direcci&oacute;n de Apoyo: <strong> <%if(eje!=null){%><%=eje.getNombre()%><%}else{%>-<%}%></strong></label>
           <p>  
           ...
           &nbsp;&nbsp;
           <input type="submit" value="Aceptar" class="btn btn-mini btn-success"/>
     ...
     <div class="span4" style="<%if(eje!=null){ out.print("display: normal"); }else{ out.print("display: none");}%> ">
        ...
        <form id="buscar" name="buscar"  action="<%=request.getContextPath()%>/IndicadorApoyo" >
    ...
    <blockquote>
    <p class="text-success">Reporte de indicadores de <strong><%=eU.getNombre()%></strong></p>  
    <small>Unidad de apoyo <strong><%=faU.getNombre()%></strong> </small>
...
<tbody>
                ...
                <%-- META inicio--%> 
                        ...
                        <td>                            
                       <% 
                    sge.modelo.Usuario wr = null; wr = (sge.modelo.Usuario) request.getSession().getAttribute("listaPerfilUsuario");
                    int idEjeEstrategicoX=wr.getIdejeestrategico();
                       %>
                       <% Periodo pdo=null;                        
                       pdo=(Periodo)request.getSession().getAttribute("SesPeriodoVigente");                       
                       %>                       
                            <%{   %>  
                   <form  class="claseform" name="claseform" id="claseform" onsubmit="return validarNumeroMeta(<%=in.getNro()%>)" action="<%=request.getContextPath()%>/IndicadorApoyo" method="POST">
                        <center>
                            <p></p>
                            <p></p>
...
<%}
 ...
 <%}%>
                        ...
                        <%if(in.getEstadometa()!=0){%>
                        <input type="submit" onmouseover="formindicador(<%=in.getNro()%>);" name="btnmeta<%=in.getNro()%>" id="btnmeta<%=in.getNro()%>"  value="Aceptar" <% if(in.getEstadometa()==0){out.print("disabled");}%> class="btn btn-mini btn-success"/>
                        <%}%>                   
                        <%}%>                       
                        </center>
                         </form>
         ...
         <%-- META FIN--%>  
            ...
            <%-- INICIO avance--%>
                    ...
                    <td>
                        ...
                        %>                        
                      <form class="claseform1" name="claseform1" action="<%=request.getContextPath()%>/IndicadorApoyo" method="POST">    
                        <center>         
                    ...
                    <td >
                        <form class="claseform2" id="claseform2<%=in.getNro()%>" name="claseform2" action="<%=request.getContextPath()%>/IndicadorApoyo" method="POST">    
                        <center>
            ...
            <input class="span5" id="prependedInput<%=in.getNro()%>" name="avance<%=in.getNro()%>" type="text" placeholder="" value="<%if(av!=null){%><%if(in.getIdmeta()==av.getIdmeta()){%><%=av.getMeta() %><%}else{%>0<%}%><%}else{%>0<%}%> ">
             <span class="add-on">%</span>
            </div>    
            <%}else{%>
            ...
            <input type="hidden"  name="valor" value="<%if(name!=null){%><%=name.getNombre()%><%}%>" >
                      <input type="hidden"  name="nro_avance<%=in.getNro()%>" value="<%if(nrometa!=null){%><%=nrometa.getId_ciclo() %><%}else{%>0<%}%>" >
                        ...
                        <input type="hidden"  name="idavance<%=in.getNro()%>" value="<%if(av!=null){%><%=av.getIdavance()%><%}else{%>0<%}%>" > 
                          <%if(in.getMeta()!=0){%>
                         <%if(in.getEstadoavance()!=0){%>
                        <input type="button" onclick="validarNumeroAvance(<%=in.getNro()%>)" onmouseover="formindicador_2(<%=in.getNro()%>)" id="btnavance<%=in.getNro()%>" name="btnavance<%=in.getNro()%>" value="Aceptar" <%if(nrometa!=null){if(in.getNro()==nrometa.getIdmeta()){ }else{out.print("disabled");}}else{out.print("disabled");}%> class="btn btn-mini btn-success"/>
                        <%}%>
                        <%}%>
                ...
                <p></p>
                 <br>
                    ...
                    <%if(in.getTotalavance()!=0){%>  
                         ...
                         <form class="claseform4" name="claseform4" action="<%=request.getContextPath()%>/IndicadorApoyo?opt=21&idfilial=<%if(fiU!=null){%><%=fiU.getIdfilial()%><%}else{%>0<%}%>&ideapfacultad=<%if(eU!=null){%><%=eU.getIdeapfacultad()%><%}else{%>0<%}%>&idfilialfacultad=<%if(faU!=null){%><%=faU.getIdfilialfacultad()%><%}else{%>0<%}%>&idperiodometa=<%if(idperidoM!=null){%><%=idperidoM.getIdperiodometa()%><%}else{%>0<%}%>" method="POST">    
