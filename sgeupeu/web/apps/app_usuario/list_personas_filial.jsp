<%-- 
    Document   : list_personas_filial
    Created on : 29/09/2013, 12:43:03 PM
    Author     : Edwin
--%>
 ...
 $('.toltipx').tooltip();
    ...
    <body>
		...
		<div class="span12">
            ...
            <a href="<%=request.getContextPath()%>/Usuario?opt=42" class="btn" ><div class="icon-plus"></div>&nbsp;Reg. Persona</a>
             </div>
      ...
      <tbody>
                <%
                        int x=0;     
                        List<Persona> lista=(List<Persona>) request.getSession().getAttribute("listaPersonasFilial");
                        if (lista != null) {
            ...
            <a class="btn btn-small toltipx" title="Editar Datos Personales" href="<%=request.getContextPath()%>/Usuario?opt=47&idPersona=<%=us.getIdpersona()%>"><div class="icon-edit"></div></a>
            <a id="eliminar" class="btn btn-small toltipx" title="Desea Realmente Eliminar porque ya no es reversible ??" href="<%=request.getContextPath()%>/Usuario?opt=46&idPersona=<%=us.getIdpersona()%>"><div class="icon-remove"></div></a>          
